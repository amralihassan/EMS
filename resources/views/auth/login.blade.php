...
<link rel="stylesheet" type="text/css" href="{{asset($css_path .'components.css')}}">
  ...
  {{-- <link rel="stylesheet" type="text/css" href="{{asset($css_path .'custom-rtl.css')}}"> --}}
  <!-- END MODERN CSS-->
  <!-- BEGIN Page Level CSS-->
  <link rel="stylesheet" type="text/css" href="{{asset($css_path .'core/menu/menu-types/vertical-menu-modern.css')}}">
  <link rel="stylesheet" type="text/css" href="{{asset($css_path .'core/colors/palette-gradient.css')}}">
  <link rel="stylesheet" type="text/css" href="{{asset($css_path .'pages/login-register.css')}}">
  <!-- END Page Level CSS-->
  <!-- BEGIN Custom CSS-->
  @if (session('lang') == 'ar')
            ...
            <div class="col-md-4 col-10 box-shadow-2 p-0">
              <div class="card border-grey border-lighten-3 m-0">
                ...
                <div class="card-content">
                  <div class="card-body">
                    <form class="form-horizontal form-simple" action="{{route('signin')}}" method="POST" novalidate>
                        @csrf
                      <fieldset class="form-group position-relative has-icon-left mb-0">
                        <input type="text" class="form-control form-control-lg input-lg" name="username" placeholder="{{ trans('local.username') }}"
                        required>
                      ...
                      <fieldset class="form-group position-relative has-icon-left">
                        ...
                        placeholder="{{ trans('local.password') }}" required>
                        <div class="form-control-position">
                          <i class="la la-key"></i>
                        </div>
                      </fieldset>
                      <div class="form-group row">
                        <div class="col-md-6 col-12 text-center text-md-left">
                          <fieldset>
                            ...
                            <label for="remember-me"> {{ trans('local.remember_me') }}</label>
                          </fieldset>
                        </div>
                        {{-- <div class="col-md-6 col-12 text-center text-md-right"><a href="recover-password.html" class="card-link">Forgot Password?</a></div> --}}
                      </div>
                      <button type="submit" class="btn btn-info btn-lg btn-block"><i class="ft-unlock"></i> {{ trans('local.login') }}</button>
                    </form>
                  </div>
                </div>
              </div>
