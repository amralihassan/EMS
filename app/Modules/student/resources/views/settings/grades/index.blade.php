...
@endsection
@section('title',trans('student::local.grades'))
@section('content')
<div class="content-header row">
    <div class="content-header-left col-md-6 col-12 mb-2">
      <h3 class="content-header-title">{{ trans('student::local.grades') }}</h3>
      <div class="row breadcrumbs-top">
        <div class="breadcrumb-wrapper col-12">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{route('students.dashboard')}}">{{ trans('local.dashboard') }}</a></li>
            <li class="breadcrumb-item active">{{ trans('student::local.grades') }}</li>
          </ol>
        </div>
      </div>
    </div>
    ...
    $(function () {
        ...
        @include('admin.datatables._config')
            buttons: [
                // new btn
                {
                    "text": "{{trans('student::local.add_grade')}}",
                ...
                // delete btn
                @include('student::settings.grades.includes._delete-btn')
                // default btns
                @include('admin.datatables._buttons')
            ],
            ajax: "{{ route('grades.index') }}",
            // columns
            @include('student::settings.grades.includes._columns')
