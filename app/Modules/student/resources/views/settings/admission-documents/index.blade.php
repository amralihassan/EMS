...
@endsection
@section('title',trans('student::local.admission_documents'))
@section('content')
<div class="content-header row">
    <div class="content-header-left col-md-6 col-12 mb-2">
      <h3 class="content-header-title">{{ trans('student::local.admission_documents') }}</h3>
      <div class="row breadcrumbs-top">
        <div class="breadcrumb-wrapper col-12">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{route('students.dashboard')}}">{{ trans('local.dashboard') }}</a></li>
            <li class="breadcrumb-item active">{{ trans('student::local.admission_documents') }}</li>
          </ol>
        </div>
      </div>
    </div>
    ...
    $(function () {
        ...
        @include('admin.datatables._config')
            buttons: [
                // new btn
                {
                    "text": "{{trans('student::local.add_admission_document')}}",
                ...
                // delete btn
                @include('student::settings.admission-documents.includes._delete-btn')
                // default btns
                @include('admin.datatables._buttons')
            ],
            ajax: "{{ route('admission-documents.index') }}",
            // columns
            @include('student::settings.admission-documents.includes._columns')
