...
@endsection
@section('title',trans('student::local.subjects'))
@section('content')
<div class="content-header row">
    <div class="content-header-left col-md-6 col-12 mb-2">
      <h3 class="content-header-title">{{ trans('student::local.subjects') }}</h3>
      <div class="row breadcrumbs-top">
        <div class="breadcrumb-wrapper col-12">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{route('students.dashboard')}}">{{ trans('local.dashboard') }}</a></li>
            <li class="breadcrumb-item active">{{ trans('student::local.subjects') }}</li>
          </ol>
        </div>
      </div>
    </div>
                    ...
                    <table id="dynamic-table" class="table data-table" >
                        <thead>
                            <tr>
                                ...
                                <th>#</th>
                                <th>{{trans('student::local.subject_name')}}</th>
                                <th>{{trans('student::local.shortcut')}}</th>
    ...
    $(function () {
        ...
        @include('admin.datatables._config')
            buttons: [
                // new btn
                {
                    "text": "{{trans('student::local.add_subject')}}",
                ...
                // delete btn
                @include('student::settings.subjects.includes._delete-btn')
                // default btns
                @include('admin.datatables._buttons')
            ],
            ajax: "{{ route('subjects.index') }}",
            // columns
            @include('student::settings.subjects.includes._columns')
