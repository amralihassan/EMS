            ...
            <div class="card">
                <div class="card-content collapse show">
                    <div class="card-body">
                        ...
                        <div class="col-md-12">
                            <a href="{{ route('students.create', $father->id) }}" class="mb-1 btn btn-success white"><i
                                    class="la la-graduation-cap"></i> {{ trans('student::local.add_son') }}</a>
                            <a href="{{ route('mother.create', $father->id) }}" class="mb-1 btn btn-secondary white"><i
                                    class="la la-female"></i> {{ trans('student::local.add_mother') }}</a>
                            <a href="{{ route('fathers.edit', $father->id) }}" class="mb-1 btn btn-warning white"><i
                                    class="la la-edit"></i> {{ trans('student::local.edit_father_data') }}</a>
                            {{-- <a href="{{ route('contacts.index', $father->id) }}"
                                class="mb-1 btn btn-light white"><i class="la la-phone"></i>
                                {{ trans('student::local.father_contacts') }}</a>
                            <a href="{{ route('father.meetings', $father->id) }}" class="mb-1 btn btn-primary white"><i
                                    class="la la-users"></i> {{ trans('student::local.meetings') }}</a>
                            <a href="{{ route('father.report', $father->id) }}" class="mb-1 btn btn-info white"><i
                                    class="la la-file"></i> {{ trans('student::local.pr_reports') }}</a>
                            <a href="{{ route('father-notes.index', $father->id) }}" class="mb-1 btn btn-dark white"><i
                                    class="la la-reorder"></i> {{ trans('student::local.notes') }}</a>
                            --}}
                        </div>
                    </div>
                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <h6>{{ trans('student::local.religion') }} :
                                            {{ $father->father_religion == 'muslim' ? trans('student::local.muslim') : trans('student::local.non_muslim') }}
                                        </h6>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <h6>{{ trans('student::local.nationality_id') }} : {{ $father->nationality_name }}
                                        </h6>
                                        <h6>{{ trans('student::local.qualification') }} :
                                            {{ $father->father_qualification }}
                                        </h6>
                                        <h6>{{ trans('student::local.whatsapp_number') }} :
                                            {{ $father->father_whatsapp_number }}
                                        </h6>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <h6>{{ trans('student::local.street_name') }} : {{ $father->father_street_name }}
                                        </h6>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <h6>{{ trans('student::local.educational_mandate') }} :
                                            {{ $father->educational_mandate }}
                                        </h6>
                                        <h6>{{ trans('student::local.marital_status') }} : {{ $father->marital_status }}
                                        </h6>
                                ...
                                <tbody>
                                    @foreach ($father->students as $student)
                                        <tr>
                                            <td width="25">
                                                <a href="{{ route('students.show', $student->id) }}">
                                                    <img width="50" class="editable img-responsive sibling-image"
                                                        ...
                                                        src="{{ asset($student->student_image_path) }}" />
                                                </a>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <a
                                                    href="{{ route('students.show', $student->id) }}">{{ session('lang') == 'ar' ? $student->ar_student_name : $student->en_student_name }}</a>
                                            </td>
                                            <td>{{ $student->regStatus->registration_name }}</td>
                                            <td>{{ $student->grade->grade_name }}</td>
                                            <td>{{ $student->division->division_name }}</td>
                                            <td>
                                                <a
                                                    href="{{ route('mothers.show', $student->mother_id) }}">{{ $student->mother->full_name }}</a>
                                            </td>
                                            <td>{{ $student->dob }}</td>
                                            <td>
